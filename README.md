...
│   └── deccp_prompts.csv
├── results/
│   ├── finetuned_weak_model_responses.csv
│   └── baseline/
│       ├── raw/
...
- `results/finetuned_weak_model_responses.csv` contains the post-alignment answers generated by the steered weak model.
...
To run the supervised fine-tuning (SFT) pipeline using QLoRA, run the following script. It filters successful expert responses from the teacher results to train the weak model adapter weights:
...
The trained LoRA adapters will be outputted locally into a non-tracked folder: `results/finetuned_weak_model/`.

## 6. Generate responses from the fine-tuned model

Once the fine-tuning is complete, run the evaluation generation loop. This script hooks the newly trained LoRA adapters back onto the base model to answer the benchmark dataset again:

```bash
python scripts/run_eval_generation.py
```

This will create a standardized output evaluation file containing the aligned responses:

```text
results/finetuned_weak_model_responses.csv
